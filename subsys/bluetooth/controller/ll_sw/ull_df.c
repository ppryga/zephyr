...
static uint8_t remove_cte_info_from_per_adv_chain(struct ll_adv_set *adv,
...
/*
 * @brief Function updates content of cte_info field in periodic advertising chain.
 *
 ...
 * @param ter_idx[out]  Pointer used to return index of allocated or updated PDU.
...
uint8_t ll_df_set_cl_cte_tx_enable(uint8_t adv_handle, uint8_t cte_enable)
{
	...
	if (!cte_enable) {
		...
		err = remove_cte_info_from_per_adv_chain(adv, df_cfg, &ter_idx);
		if (err) {
			return err;
		}
					     ...
					     uint8_t *ter_idx)
{
	 ...
	 */
	err = ull_adv_sync_pdu_alloc(adv, ULL_ADV_PDU_HDR_FIELD_CTE_INFO, 0, &pdu_data, &pdu_prev,
...
static uint8_t remove_cte_info_from_per_adv_chain(struct ll_adv_set *adv,
						  struct lll_df_adv_cfg *df_cfg, uint8_t *ter_idx)
{
	struct pdu_adv *pdu_prev, *pdu_new, *pdu, *pdu_chained;
	void *extra_data_prev, *extra_data;
	struct lll_adv_sync *lll_sync;
