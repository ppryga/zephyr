...
uint8_t ull_adv_sync_pdu_init(struct pdu_adv *pdu, uint8_t ext_hdr_flags)
{
...
uint8_t ll_adv_sync_param_set(uint8_t handle, uint16_t interval, uint16_t flags)
{
	...
	do {
		...
		ull_adv_sync_pdu_init(test_pdu, ULL_ADV_PDU_HDR_FIELD_AUX_PTR);
	...
	do {
		...
		test_pdu = lll_adv_pdu_alloc_pdu_adv();
		/* Initialize new empty PDU */
		ull_adv_sync_pdu_init(test_pdu, 0);
		/* Add some AD for testing */
		adv_sync_pdu_ad_data_set(test_pdu, tmp, strlen(tmp));
		/* Link to 1st PDU */
		lll_adv_pdu_linked_append(test_pdu, ter_pdu);
				      ...
				      void *value)
{
	...
	ter_dptr_prev = &ter_com_hdr_prev->ext_hdr.data[0];
	...
	// 	ter_hdr = ter_com_hdr->ext_hdr;
	// }
	ter_dptr = &ter_com_hdr->ext_hdr.data[0];
