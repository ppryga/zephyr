...
/* @brief Minimum antennae number required if antenna switching is enabled */
...
struct df_ant_cfg {
	uint8_t ant_num;
	/* Maximum supported antenna switch pattern length */
	uint8_t max_ant_sw_pattern_num;
	/* Selection of GPIOs to be used to switch antennae by Radio */
...
/* @brief Function performs steps related with DF antennae configuration.
 *
 * Validates DTS related with DF antennae.
 * Sets up DF related PSEL.DFEGPIO registers to give possibility to Radio
 * to drivee antennae switches.
 *
...
int radio_df_ant_configure(void)
{
	 ...
	 * for given antennae number. Confirm also that antennae number is not
	 * lower than required minimum.
	 */
 ...
 *
 * The number of antennae is hardware defined. It is provided via device tree.
 *
...
void radio_df_cte_inline_set(uint8_t enable)
{
	...
	NRF_RADIO->CTEINLINECONF |= ((enable << RADIO_CTEINLINECONF_CTEINLINECTRLEN_Pos)
