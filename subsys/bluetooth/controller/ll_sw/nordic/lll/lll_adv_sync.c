...
static inline void switch_radio_complete_and_b2b_tx(struct lll_adv_sync *lll, uint8_t phy_s);
static inline void switch_radio_complete_and_phy_end_disable(struct lll_adv_sync *lll);

#if defined(CONFIG_BT_CTLR_ADV_SYNC_PDU_BACK2BACK)
static void isr_tx(void *param);
static inline void pdu_b2b_update(struct lll_adv_sync *lll, struct pdu_adv *pdu,
				  uint32_t cte_len_us);
static inline void pdu_b2b_aux_ptr_update(struct pdu_adv *pdu, uint8_t phy, uint8_t flags,
					  uint8_t chan_idx, uint32_t offset_us,
					  uint32_t cte_len_us);
...
static void isr_tx(void *param)
{
	...
	radio_gpio_pa_lna_enable(radio_tmr_tifs_base_get() + ADV_SYNC_PDU_B2B_AFS - 4 + cte_len_us -
				 radio_tx_chain_delay_get(lll->phy_, 0) -
...
#endif /* CONFIG_BT_CTRL_DF_ADV_CTE_TX */
	{
