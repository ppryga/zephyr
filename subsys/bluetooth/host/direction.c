...
struct bt_le_df_ant_info {
	/* Bitfiled holding optional switching and sampling rates */
...
/* @brief Function sets CTE parameters for connectionless transmission
 *
 * @param[in] params    Pointer to CTE parameters object
 *
				...
				const struct bt_le_df_adv_cte_tx_params *params)
{
	...
	if (params->cte_type == BT_HCI_LE_AOD_CTE_1US ||
		...
		__ASSERT_NO_MSG((sizeof(*cp) + params->num_ant_ids) <
				UINT8_MAX);
	}
 ...
 * @return Zero in case of success, other value in case of failure.
 */
static int bt_le_df_set_adv_cte_tx_enabled(struct bt_le_ext_adv *adv,
					   bool enable)
{
	int err;
...
{
	__ASSERT_NO_MSG(adv);
	return bt_le_df_set_adv_cte_tx_enabled(adv, true);
}

int bt_le_df_adv_cte_tx_disable(struct bt_le_ext_adv *adv)
{
	__ASSERT_NO_MSG(adv);
	return bt_le_df_set_adv_cte_tx_enabled(adv, false);
}
